...
class RedisTrib
    ...
    def check_cluster_health(opt = {})
        ...
        return '{"code":1,"message":"Nodes don\'t agree about configuration"}' if !is_config_consistent?

        open_slots = []
        @nodes.each {|n|
            if n.info[:migrating].size > 0
                return '{"code":2,"message":"Some slots in migrating state"}'
            end
            if n.info[:importing].size > 0
                return '{"code":3,"message":"Some slots in importing state"}'
            end
        }
        ...
        if slots.length == ClusterHashSlots
            ...
            return '{"code":4,"message":"Not all slots are covered by nodes"}'
        end
    end
